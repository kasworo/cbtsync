	...
	<nav class="mt-2">
		...
		<li class="nav-item has-treeview">
			...
			<ul class="nav nav-treeview">
				...
				<li class="nav-item">
					<a href="index.php?p=sesi" class="nav-link">
		...
		<?php
			$qsetuji=$conn->query("SELECT*FROM tbujian u INNER JOIN tbjadwal jd USING(idujian) LEFT JOIN  tbsetingujian su USING(idjadwal) WHERE u.status='1'");
			$ceksetuji=$qsetuji->num_rows;
			if($ceksetuji>0):
		?>
		<li class="nav-item has-treeview">
			<a href="#" class="nav-link">
...
<?php elseif($level=='2'):?>
	<nav class="mt-2">
		<ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
			...
			<?php
				$qsetuji=$conn->query("SELECT*FROM tbsetingujian su INNER JOIN tbjadwal jd USING(idjadwal) INNER JOIN tbujian u USING(idujian) WHERE u.status='1'");
				$ceksetuji=$qsetuji->num_rows;
				if($ceksetuji>0):
			?>
			<li class="nav-item">
			...
			<li class="nav-item has-treeview">
				...
				<ul class="nav nav-treeview">					
				<?php
					$qwalas=$conn->query("SELECT*FROM tbrombel WHERE username='$_COOKIE[c_user]'");
					$cekwalas=$qwalas->num_rows;
					if($cekwalas>0):
				?>
					<li class="nav-item">
						<a href="index.php?p=hasiltes" class="nav-link">
...
<?php if($level=='2'):?>
	<nav class="mt-2">
		<ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
			<li class="nav-item">
				<a href="index.php?p=dasboard" class="nav-link">
					<i class="fas fa-dashboard-alt nav-icon"></i>
					<p>Dashboard</p>
				</a>
			</li>
			<li class="nav-item">
