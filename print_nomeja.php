<?php
	...
	class PDF extends FPDF{		
		...
		if($level){
			$sql = $conn->query("SELECT ps.idsiswa, ps.nmsiswa, ps.nis, ps.nisn, ps.tgllahir, ps.tmplahir, ps.nmpeserta, ps.passwd, r.nmrombel, u.nmujian, r1.kdruang, r1.nmruang FROM tbpeserta ps INNER JOIN tbrombelsiswa rs USING(idsiswa) INNER JOIN tbrombel r USING(idrombel) INNER JOIN tbthpel t USING(idthpel) INNER JOIN tbujian u USING(idujian) INNER JOIN tbruang r1 USING(idruang) WHERE u.status='1' AND t.aktif='1' GROUP BY ps.idsiswa ORDER BY ps.nmpeserta ASC");
		}
		else{
		...
		{	
			...
			$cell[$i][0]=substr($d['nmpeserta'],3,2).'-'.substr($d['nmpeserta'],5,4).'-'.substr($d['nmpeserta'],9,4).'-'.substr($d['nmpeserta'],-1);
			$cell[$i][1]=ucwords(strtolower($nmsiswa));
		...
		for($j=1;$j<=$hal;$j++){
			...
			for($k=1;$k<=$perhal/2;$k++){
				...
				$pdf->Cell(7.0,0.55,'PANITIA PENYELENGGARA','TR',0,'C');
				...
				$pdf->Cell(3.5,0.60,'No. Peserta','T');
				$pdf->Cell(0.25,0.60,':','T');
				$pdf->Cell(5.5,0.60,$cell[($j-1)*$perhal+$k][0],'TR');
				$pdf->Cell(0.5,0.60);
				$pdf->Cell(0.25,0.60,'','LT');
				$pdf->Cell(3.50,0.60,'No. Peserta', 'T');
				$pdf->Cell(0.25,0.60,':','T');
