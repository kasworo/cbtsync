...
$no = 0;
$baris = 6;
$qsiswa = "SELECT p.*, s.kdskul FROM tbpeserta p INNER JOIN tbskul s USING(idskul) WHERE deleted='0'";
$ceksiswa = cquery($qsiswa);
if ($ceksiswa > 0) {
  $qs = vquery($qsiswa);
  foreach ($qs as $s) {
    $no++;
    $objPHPExcel->setActiveSheetIndex(0)
      ...
      ->setCellValue("I$baris", $s['agama'])
      ->setCellValue("J$baris", $s['alamat']);
    $baris++;
  }
} else {
  while ($no < 15) {
    $no++;
    $objPHPExcel->setActiveSheetIndex(0)
      ->setCellValue("A$baris", $no)->setCellValue("B$baris", '')->setCellValue("C$baris", '')->setCellValue("D$baris", '')->setCellValue("E$baris", '')->setCellValue("F$baris", '')->setCellValue("G$baris", '')->setCellValue("H$baris", '')->setCellValue("I$baris", '')->setCellValue("J$baris", '');
    $baris++;
  }
}
$semua = $baris - 1;
