...
function getuser()
{
	$quser = "SELECT u.username, u.level FROM tbuser u WHERE username='$_COOKIE[id]'";
	$users = vquery($quser)[0];
	return $users;
}
class PDF extends FPDF
{
	function Header()
	{
		...
		$qth = "SELECT*FROM tbthpel WHERE aktif='1'";
		$setid = vquery($qth)[0];
		$thpel = substr($setid['desthpel'], 0, 9);

		$quji = "SELECT idujian, nmtes FROM tbujian u INNER JOIN tbtes t USING(idtes) WHERE status='1'";
		$uji = vquery($quji)[0];
...
foreach ($ds as $d) {
	...
	$pdf->Ln(0.75);
	$pdf->SetFont('Times', '', '11');
	$qjd = "SELECT jd.* FROM tbjadwal jd INNER JOIN tbujian u USING(idujian) WHERE u.status='1'";
	$cekjd = cquery($qjd);
	$dtjd = vquery($qjd);
	$i = 0;
	foreach ($dtjd as $jd) {
		$i++;
		// $qses = "SELECT*FROM tbsesiujian su WHERE su.idjadwal='$jd[idjadwal]' AND su.idsiswa='$d[idsiswa]'";
		// $ses = vquery($qses)[0];
		$cell[$i][0] = $i . '.';
