...
function GetMapel($r)
{
	...
	$sqlp = "SELECT pg.idmapel, mp.akmapel FROM tbpengampu pg INNER JOIN tbgtk g USING(idgtk) INNER JOIN tbmapel mp USING(idmapel) INNER JOIN tbrombel rb USING(idrombel)INNER JOIN tbthpel tp USING(idthpel) WHERE tp.aktif='1' AND pg.idrombel='$r'";
	return vquery($sqlp);
}

function GetStatistik($rb, $mp, $u)
{
	$sqlsta = "SELECT MAX(n.nilai) as maks, MIN(n.nilai) as mins, AVG(n.nilai) as rata FROM tbpeserta ps INNER JOIN tbujian u USING(idujian) LEFT JOIN tbnilai n using(idsiswa,idujian) LEFT JOIN tbmapel mp USING(idmapel) LEFT JOIN tbrombelsiswa rs USING(idsiswa) INNER JOIN tbrombel rb USING(idrombel) WHERE rs.idrombel='$rb' AND n.idmapel='$mp' AND u.idujian='$u' GROUP BY n.idmapel, rs.idrombel";
	if (cquery($sqlsta) > 0) {
		$sta = vquery($sqlsta)[0];
		$nilai = array(
			'maks' => $sta['maks'],
...
class PDF extends FPDF
{
	function Header()
	{
		...
		$this->Cell(1.75, 0.5, '');
		$this->Cell(29.25, 0.5, 'REKAPITULASI HASIL PENILAIAN', 0, 0, 'C', 0);
		$this->Ln();
		$this->Cell(1.75, 0.5, '');
	...
	function IsiData($id, $hal, $uji)
	{
		...
		if ($hal == 2) {
			...
			$j = 0;
			foreach ($qmp as $mp) {
				$sta = GetStatistik($id, $mp['idmapel'], $uji);
				//var_dump($sta);
