		...
		<div class="modal-body">
			...
			<div class="form-group row mb-2">			
				...
				<select class="form-control form-control-sm col-sm-6" id="idrombel" name="idrombel">
					<option value="">..Pilih..</option>
					<?php
					$qrb=$conn->query("SELECT r.* FROM tbrombel r INNER JOIN tbthpel t USING(idthpel) WHERE t.aktif='1'");
	...
	<div class="card card-secondary card-outline">
		...
		<div class="card-body">
			<div class="table-responsive">
				...
				<tbody>
				<?php
					$qk=$conn->query("SELECT a.idampu, m.nmmapel, r.nmrombel, u.nama FROM tbpengampu a INNER JOIN tbrombel r USING(idrombel) INNER JOIN tbmapel m USING(idmapel) INNER JOIN tbuser u ON a.username=u.username WHERE r.idthpel='$idthpel' ORDER BY idrombel, idmapel");
					$no=0;
...
<script type="text/javascript">
	...
	$("#simpan").click(function(){
		var ida=$("#idampu").val();
		var kls=$("#idkelas").val();
		var idr=$("#idrombel").val();
		var idm=$("#idmapel").val();
		var idg=$("#idguru").val();
		$.ajax({
			url:"pengampu_simpan.php",
			type:'POST',
			data:"aksi=simpan&id="+ida+"&mp="+idm+"&gr="+idg+"&rm="+idr+"&kl="+kls,
	...
	$("#salin").click(function(){
		var kls=$("#idklsasl").val();
		var idrasl=$("#idrombasl").val();
		var idrtjn=$("#idrombtjn").val();
		$.ajax({
		url:"pengampu_simpan.php",
		type:'POST',
